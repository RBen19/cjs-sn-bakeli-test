...
watch(() => props.membre, (m) => {
  if (m) Object.assign(form, {
    prenom: m.prenom || '', nom: m.nom || '', email: m.email || '',
    telephone: m.telephone || '', ville: m.ville || '',
    date_naissance: m.date_naissance ? m.date_naissance.substring(0, 10) : '',
    competences: m.competences || '', statut: m.statut || 'actif',
  })
}, { immediate: true })

async function handleSubmit() {
  if (!props.membre) return
  loading.value = true
  errors.value = {}
  try {
    await membresStore.updateMembre(props.membre.id, { ...form })
    emit('success', 'Membre mis à jour avec succès !')
    emit('close')
  } catch (error) {
          ...
          <form @submit.prevent="handleSubmit" class="px-6 py-5">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input v-model="form.telephone" type="text" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-white"/>
              </div>
              <div>
              ...
              <div class="col-span-2">
                ...
                <textarea v-model="form.competences" rows="3" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-white resize-none"></textarea>
              </div>
              <div class="col-span-2">
